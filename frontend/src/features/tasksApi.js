  ...
  endpoints: (builder) => ({
    ...
    addTask: builder.mutation({
      ...
      async onQueryStarted(task, { dispatch, queryFulfilled }) {
        ...
        try { 
          ...
          console.log("Network failed, Workbox queued the Add task.");
        }
      },
      invalidatesTags: ['Task'],
    }),
      ...
      async onQueryStarted({ id, ...patch }, { dispatch, queryFulfilled }) {
        const patchResult = dispatch(
          tasksApi.util.updateQueryData('getTasks', 1, (draft) => {
            ...
            if (task) Object.assign(task, patch); // Instantly update the UI
          })
        );
        try { 
          await queryFulfilled; 
        } catch { 
          console.log("Network failed, Workbox queued the Update task.");
        }
      },
      invalidatesTags: ['Task'],
    }),
    ...
    deleteTask: builder.mutation({
      ...
      async onQueryStarted(id, { dispatch, queryFulfilled }) {
        const patchResult = dispatch(
          tasksApi.util.updateQueryData('getTasks', 1, (draft) => {
            draft.tasks = draft.tasks.filter(t => t._id !== id); // Instantly remove from UI
            draft.totalTasks -= 1;
          })
        );
        try { 
          ...
          console.log("Network failed, Workbox queued the Delete task.");
        }
      },
      invalidatesTags: ['Task'],
    }),
